...
const reducer = createReducer<State.EditOrderState, Actions.EditOrederActions>(
  ...
  on(Actions.upsertHostingOrderItem, (state, { productId }) => ({
    ...state,
    hostingItem: { ...state.hostingItem, productId }
  }))
);

export function EDIT_ORDER_REDUCER(
  state: State.EditOrderState,
  action: Actions.EditOrederActions
